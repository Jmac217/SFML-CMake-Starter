...
namespace Mac {
	...
	Mac::EntityManager* SystemManager::GetEntityManager()
	{
		return m_entityManager;
	}

	Mac::MessageHandler* SystemManager::GetMessageHandler()
	{
		return &m_messages;
	}
	...
	void SystemManager::Update(float l_deltaTime)
	{
	...
	void SystemManager::EntityModified(const EntityId& l_entity, const Bitmask& l_bits)
	{
		for (auto& s_itr : m_systems)
		{
			...
			else if (system->HasEntity(l_entity))
