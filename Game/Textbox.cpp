...
namespace Mac {
	...
	void Textbox::Setup(int l_visible, int l_charSize, int l_width, sf::Vector2f l_screenPos)
	{
		...
		sf::Vector2f l_offset(2.0f, 4.0f);

		m_font.loadFromFile("Fonts/rainyhearts.ttf");
		...
		m_backdrop.setSize(sf::Vector2f(
			l_width,
			(l_visible * (l_charSize * 1.2f))
		));
		m_backdrop.setFillColor(sf::Color(90, 90, 200, 90));
