	...
	void Character::Load(const std::string& l_path)
	{
		...
		while (std::getline(file, line))
		{
			...
			else if (type == "BoundingBox")
			{
				...
				keystream
					>> m_attackAABBoffset.x
					>> m_attackAABBoffset.y
					>> m_attackAABB.width
					>> m_attackAABB.height;
			}
			else if (type == "Speed")
	...
	void Character::Update(float l_deltaTime)
	{
		...
		if (m_attackAABB.width != 0	&& m_attackAABB.height != 0)
		{
			...
			if (m_entityManager->GetContext()->m_debugOverlay.Debug())
			{
				...
				arect->setFillColor(sf::Color(255, 0, 0, (m_state == EntityState::Attacking && m_spriteSheet.GetCurrentAnim()->IsInAction() ? 200 : 100)));
